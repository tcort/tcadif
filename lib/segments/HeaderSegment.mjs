...
class HeaderSegment extends Segment {
    ...
    static fromADI(s) {
        const headerSegment = new HeaderSegment();
        return Segment.fromADI(s, headerSegment);
    }

    toADI() {
        const adi = super.toADI().trim();
        if (adi === EndOfHeaderTag.toADI()) {
            return HeaderSegment.factory().toADI();
        }
        return adi;
    }
